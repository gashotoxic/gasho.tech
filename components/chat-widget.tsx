    ...
    <div ref={widgetRef} className="fixed bottom-6 right-6 z-[10000]">
      ...
      {!isOpen && (
        <button
          ...
          className="relative w-14 h-14 rounded-full bg-gradient-to-br from-[#1abc9c] to-[#16a085] text-white flex items-center justify-center cursor-pointer
            shadow-lg shadow-[#1abc9c]/25 hover:shadow-xl hover:shadow-[#1abc9c]/40 hover:scale-110 active:scale-95
            transition-all duration-300 ease-out group"
          aria-label="Open chat"
        >
          <MessageCircle className="w-6 h-6 group-hover:scale-110 transition-transform duration-200" />
          {/* Notification dot */}
          <span className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-white dark:border-[#1a1a1a] animate-pulse" />
        </button>
      )}
      ...
      {isOpen && (
        <div
          className="absolute bottom-20 right-0 w-[340px] sm:w-[380px] h-[520px] flex flex-col overflow-hidden rounded-2xl
            bg-white dark:bg-[#1e1e1e]
            border border-gray-200/80 dark:border-gray-700/60
            shadow-2xl shadow-black/10 dark:shadow-black/40"
          style={{ animation: "chatSlideIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)" }}
        >
          {/* Header */}
          <div className="bg-gradient-to-r from-[#1abc9c] via-[#18b893] to-[#0e8c75] text-white px-5 py-4 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm">
                <Sparkles className="w-4.5 h-4.5" />
              </div>
              <div>
                <p className="font-semibold text-sm tracking-normal leading-tight">GashoTech Assistant</p>
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/80 dark:bg-[#141414]">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`flex gap-2 ${msg.sender === "bot" ? "" : "flex-row-reverse"}`}
              >
                {/* Avatar */}
                <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                  msg.sender === "bot"
                    ? "bg-[#1abc9c]/15 dark:bg-[#1abc9c]/20"
                    : "bg-gray-200 dark:bg-gray-700"
                }`}>
                  {msg.sender === "bot"
                    ? <Bot className="w-3.5 h-3.5 text-[#1abc9c]" />
                ...
                <div className="flex flex-col max-w-[78%]">
                  <div
                    className={`px-3.5 py-2.5 text-sm leading-relaxed ${
                      msg.sender === "bot"
                        ? "bg-white dark:bg-[#2a2a2a] text-gray-800 dark:text-gray-100 rounded-2xl rounded-tl-md shadow-sm border border-gray-100 dark:border-gray-700/60"
                        : "bg-[#1abc9c] text-white rounded-2xl rounded-tr-md shadow-sm"
                    }`}
            ...
            {typing && (
              <div className="flex gap-2">
                <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 bg-[#1abc9c]/15 dark:bg-[#1abc9c]/20">
                  <Bot className="w-3.5 h-3.5 text-[#1abc9c]" />
                </div>
                <div className="flex items-center gap-1.5 px-4 py-3 rounded-2xl rounded-tl-md bg-white dark:bg-[#2a2a2a] shadow-sm border border-gray-100 dark:border-gray-700/60">
                  {[0, 1, 2].map((i) => (
                    <span
                      key={i}
          ...
          {/* Suggestions (only show when just welcome) */}
          {messages.length <= 1 && !typing && (
            <div className="px-4 pb-2 pt-1 flex flex-wrap gap-1.5 border-t border-gray-100 dark:border-gray-800">
              {suggestions.map((s) => (
                <button
                  key={s}
                  onClick={() => {
          ...
          <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1e1e1e] shrink-0">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={sendMessage}
                disabled={!input.trim()}
                className="w-10 h-10 rounded-full bg-[#1abc9c] hover:bg-[#16a085] hover:scale-110 active:scale-95 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white flex items-center justify-center transition-all duration-200 shrink-0 disabled:cursor-not-allowed disabled:hover:scale-100"
